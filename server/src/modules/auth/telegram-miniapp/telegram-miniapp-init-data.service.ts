...
export const verifyTelegramInitData = (
    ...
    if (!/^[a-f0-9]{64}$/i.test(hash)) return { ok: false, reason: 'malformed hash' };
    params.delete('hash');
    ...
    if (!/^\d+$/.test(authDateRaw) || !Number.isSafeInteger(authDateSeconds)) return { ok: false, reason: 'malformed auth_date' };
    ...
    if (!parsedUser || typeof parsedUser.id !== 'number' || !Number.isSafeInteger(parsedUser.id) || parsedUser.id <= 0) return { ok: false, reason: 'missing user.id' };
