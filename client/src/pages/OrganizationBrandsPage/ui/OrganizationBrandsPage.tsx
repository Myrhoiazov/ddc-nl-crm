...
const OrganizationBrandsPage = () => {
    ...
    return <Page>
        <div className={s.header}>
            <div><h1>Организация и бренды</h1><p>Юридическое лицо и торговые названия для инвойсов.</p></div>
            <button onClick={syncMollie}>Синхронизировать Mollie</button>
        </div>

        <section className={s.card}>
            <h2>Юридическая организация</h2>
            ...
            <button className={s.primary} disabled={saving} onClick={saveOrganization}>Сохранить реквизиты</button>
        </section>

        <section className={s.card}>
            <h2>{brandForm.id ? `Редактировать ${brandForm.name}` : 'Добавить бренд / проект'}</h2>
            <div className={s.grid}>
                <label>Название *<input value={brandForm.name} onChange={(e) => setBrandForm({ ...brandForm, name: e.target.value })} /></label>
                <label>Slug *<input placeholder="talent-center-ddc" value={brandForm.slug} onChange={(e) => setBrandForm({ ...brandForm, slug: e.target.value.toLowerCase().replace(/\s+/g, '-') })} /></label>
                <label>Email<input value={brandForm.email} onChange={(e) => setBrandForm({ ...brandForm, email: e.target.value })} /></label>
                <label>Телефон<input value={brandForm.phone} onChange={(e) => setBrandForm({ ...brandForm, phone: e.target.value })} /></label>
                <label>Сайт<input value={brandForm.website} onChange={(e) => setBrandForm({ ...brandForm, website: e.target.value })} /></label>
                <label>Адрес<input value={brandForm.address} onChange={(e) => setBrandForm({ ...brandForm, address: e.target.value })} /></label>
                <label>Mollie profile ID<input value={brandForm.mollieProfileId} onChange={(e) => setBrandForm({ ...brandForm, mollieProfileId: e.target.value })} /></label>
                <label>Цвет<input type="color" value={brandForm.primaryColor} onChange={(e) => setBrandForm({ ...brandForm, primaryColor: e.target.value })} /></label>
                <label>Логотип
                ...
                <label className={s.check}><input type="checkbox" checked={brandForm.isDefault} onChange={(e) => setBrandForm({ ...brandForm, isDefault: e.target.checked })} /> Бренд по умолчанию</label>
            </div>
            {brandForm.logoUrl && <img className={s.logoPreview} src={getLogoSrc(brandForm.logoUrl)} alt={brandForm.name || 'Логотип'} />}
            <div className={s.actions}>
                <button className={s.primary} disabled={saving} onClick={saveBrand}>Сохранить бренд</button>
                {brandForm.id && <button onClick={() => setBrandForm(emptyBrand(organization.id))}>Отмена</button>}
            </div>
        </section>

        <section className={s.card}>
            <h2>Бренды и проекты</h2>
            <div className={s.brands}>{brands.map((brand) => <article className={s.brand} key={brand.id}>
                ...
                <button onClick={() => setBrandForm(brand)}>Редактировать</button>
                {brand.isActive && <button className={s.danger} onClick={() => archive(brand.id)}>Архивировать</button>}
            </article>)}</div>
        </section>
    </Page>;
};
