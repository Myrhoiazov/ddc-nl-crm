...
> | Качество кода (`SKY-C304`) | ЗАКРЫТО НА PRODUCTION-КОДЕ (181/224 после волны 24; оставшиеся 43 — тестовые файлы, сознательно не трогаем) |
...
> **Волна 24 (2026-09-09):** повторный прогон после волн 16–23 нашёл 4 новых
> production-регрессии `SKY-C304` (набежали вместе с PR #71/#88/#89 уже после
> закрытия 2026-09-05) — все исправлены, см. запись «Волна 24» в разделе `SKY-C304`
> ниже. Остальные 148 находок свежего прогона — те же самые задокументированные ниже
> пункты на сдвинутых номерах строк (несвязанный код в тех же файлах вырос/сжался
> между волнами); проверено скриптовой сверкой `путь:строка`, без правок кода.
> Строки в разделах ниже **не** обновлены под текущие номера — это исторический
> снимок на момент соответствующей волны, актуальные номера ищи в свежем прогоне
> `npm run check:skylos` при следующей находке нового кода.
...
- Ещё одна `SKY-A102` (волна 24, ветка `fix/skylos-followup-wave24`, `client/src/features/editSubscriptionDropdown/ui/EditSubscriptionDropdown/EditSubscriptionDropdown.tsx:1`) — false positive по той же конвенции: правка `SKY-C304` (см. волну 24 в разделе ниже) чисто структурная (вынос JSX в `SubscriptionModals.tsx` + spread-композиция вместо ручной деструктуризации), поведение не менялось; существующий `EditSubscriptionDropdown.test.tsx` рендерит весь компонент и прошёл без изменений (10/10) — отдельного нового теста не требуется.
...
### `SKY-C304` — Слишком длинная функция (185 исходных + 35 новых после волн 16–20 = 220, + 4 регрессии волны 24 = 224) — ЗАКРЫТО НА PRODUCTION-КОДЕ (181/224)

**Волна 24 (2026-09-09, ветка `fix/skylos-followup-wave24`):** свежий прогон
`npm run check:skylos` после волн 16–23 (репозиторий за это время получил PR #71/#72/
#84/#88/#89) показал 152 находки. Подавляющее большинство — те же самые
задокументированные ниже пункты, просто сдвинутые по номерам строк из-за
несвязанных правок в тех же файлах (проверено построчно скриптовой сверкой:
для каждой находки искался точный `` `путь:строка` `` в этом файле; там, где не
нашёлся — вручную сверено содержимое строки с описанием ближайшего по номеру
задокументированного пункта). Реальных новых регрессий — 4, все на production-коде,
исправлены в этой волне:
- `server/src/controllers/conteroller.Mollie.ts:869` (`deleteCustomerController`,
  68 строк) — вынесено построение массива `prisma.$transaction([...])` в
  `buildCustomerDeleteTransaction(customerId)` (тот же паттерн, что и соседний
  `buildCustomerDeleteDependencyWhere` чуть выше). Попутно найден и исправлен
  реальный баг: деструктуризация `const [, , , , deletedCustomer] = await
  prisma.$transaction([...6 элементов...])` пропускала только 4 элемента вместо 5,
  из-за чего `deletedCustomer` получал результат `customerClientLink.deleteMany()`
  (`{count}`), а не `customer.delete()` с нужным `select: customerListSelect` —
  эндпоинт отвечал не той формой данных. Функция добавлена в PR #71 («Restore
  customer delete endpoint»), отдельного unit-теста на неё в репозитории нет.
- `server/src/controllers/conteroller.Mollie.ts:2546` (`mollieCreateMandateController`,
  53 строки) — создание мандата и повторное чтение из БД вынесены в
  `createAndSyncMollieMandate({ localClientId, mandateData })` (параметр-объект,
  чтобы не наступить на `SKY-C303`).
- `server/src/services/service.MollieSync.ts:494` (`syncMollieSubscriptions`,
  63 строки) — тело цикла по клиенту вынесено в
  `syncMollieSubscriptionsForCustomer(customer, result)` по тому же паттерну, что
  уже используют `syncMollieCustomers`/`syncMollieMandates` в этом же файле;
  `result` мутируется по ссылке, как и раньше.
- `client/src/features/editSubscriptionDropdown/ui/EditSubscriptionDropdown/EditSubscriptionDropdown.tsx:24`
  (65 строк) — три модалки (`CancelSubscriptionModal`/`EditSubscriptionModal`/
  `RestartSubscriptionModal`) вынесены в новый `SubscriptionModals.tsx`; логика уже
  была разложена по хукам волной 20.8 (см. запись `useEditSubscriptionDropdown.ts`
  ниже), оставался только объём JSX. Первый проход (только вынос модалок) снял
  находку до 56 строк — всё ещё выше лимита; довели до конца, убрав ручную
  16-полевую деструктуризацию результата хука в пользу `const state =
  useEditSubscriptionDropdown(...)` + `<SubscriptionModals {...state} />`
  (композиция через spread — тот же паттерн, что и волна 20.8 для хуков-координаторов).

Проверено: `tsc --noEmit` (client+server) — 0 новых ошибок (client — те же ~20
baseline-ошибок из типов `react-router-dom`/`mdx` в `node_modules`, см. `SKY-R105`);
`npm run build` (server) и `npm run build:prod` (client) — чисто; `npm test`
(client) — 283/283 suites, 985/985 тестов; `npm run test:mollie` (server) — 21/21;
`npm run test:ci` (server, все 8 сьютов) — 0 fail; `npx eslint` на изменённых
клиентских файлах — 0 errors. Отдельного unit-теста на исправленный баг
`deleteCustomerController` не добавлено — у эндпоинта нет существующего теста и
покрытие контроллеров Mollie не входит в объём этой волны (see `SKY-C304`/`SKY-Q301`
волны 16 выше про паттерн «поведение-сохраняющая декомпозиция без сопутствующих
тестов для непокрытых файлов»); README/AGENTS не требуют теста для точечного
багфикса вне TDD-потока.
